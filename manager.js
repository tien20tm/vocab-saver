...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function loadHardWords() {
    chrome.storage.local.get(['vocabulary-hard-words'], function(result) {
  ...
  function saveHardWords() {
    chrome.storage.local.set({ 'vocabulary-hard-words': [...hardWordIds] });
  }
  ...
  function renderWords(words, categories, totalCount) {
    ...
    words.forEach(word => {
      ...
      hardButton.className = isHard
        ? 'text-yellow-400 hover:text-yellow-500 transition-colors p-1 rounded-full hover:bg-yellow-50'
        : 'text-gray-300 hover:text-yellow-400 transition-colors p-1 rounded-full hover:bg-yellow-50';
      hardButton.innerHTML = isHard
      ...
      hardButton.title = isHard ? 'Bỏ đánh dấu từ khó' : 'Đánh dấu từ khó';
      hardButton.addEventListener('click', function() {
        if (hardWordIds.has(word.id)) {
          hardWordIds.delete(word.id);
        } else {
          hardWordIds.add(word.id);
        }
        saveHardWords();
        renderCurrentPage();
      });
      actionButtons.appendChild(hardButton);
